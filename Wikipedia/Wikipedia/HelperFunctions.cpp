...
HttpClient^ HelperFunctions::CreateHttpClient() {
	...
	/*std::wstring s((httpCookie->Domain  + cookie)->Data());
	std::wstring stemp = std::wstring(s.begin(), s.end());
	LPCWSTR sw = stemp.c_str();
	OutputDebugString(sw);*/

	/*if (values->Lookup("cookieName") != nullptr) {
		HttpCookieManager^ cookieManager = filter->CookieManager;
		HttpCookie^ cookie = ref new HttpCookie((String^)values->Lookup("cookieName"), (String^)values->Lookup("cookieDomain"), (String^)values->Lookup("cookiePath"));
		cookie->Value = (String^)values->Lookup("cookieValue");
...
// Helper function to convert underscores into space
...
void HelperFunctions::Sleep(uint32 ms)
{
	WaitForSingleObjectEx(GetCurrentThread(), ms, FALSE);
}

String^ HelperFunctions::ProcessImages(String^ sectionStr) {
	OutputDebugString(L"Method: processimage");
...
String^ HelperFunctions::ProcessSection(String^ sectionStr, int id, int level) {
	OutputDebugString(L"Method: processection");
	wstring section = sectionStr->Data();
	...
	// Defunct since they changed shit?
	...
	if (pos != wstring::npos) {
		...
		String^ showCode = "<span class=\"mw-showsection\"><a href=\"#section" + id + "\" onclick =\"show_hide('section" + id + L"');\" title=\"Show/hide section\">" + downImage + "</a></span>" + closingTagP + "<a id=\"sectionLink" + id + "\"\><br/><div class=\"section\" id=\"section" + id + L"\">";
